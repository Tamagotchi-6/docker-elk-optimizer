...
# 2. vm.max_map_count 설정 (Linux)
sudo sysctl -w vm.max_map_count=262144

# macOS (Docker Desktop)는 아래 경로에서 설정
...
RAM이 부족한 팀원은 `docker-compose.yml`의 `ES_JAVA_OPTS` 값만 낮춰서 실행하면 됩니다.

### 공유 시 주의사항

- `.env` 파일은 커밋하지 않습니다. `.env.example`을 복사해서 사용하세요.
- ES 데이터는 `esdata` 볼륨에 저장됩니다. 초기화가 필요하면 `docker compose down -v`를 실행하세요.
- ES가 뜨는 데 40초 이상 걸릴 수 있습니다. `docker compose ps`로 `healthy` 상태를 확인하세요.
